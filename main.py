...
# Instead of from storm_database import StormDatabase, we use:
from storm_dataframes import StormDataFrames

# Instead of from wind_sql import WindSQL, etc.:
...
def main():
    ...
    # 1) Create brand-new in-memory 'DB' (actually dataframes)
    ...
    # 3) Load CSV data into DataFrames
    ...
    # 4) Instantiate the DataFrame-based classes
    wind = WindDF(db)
    tornado = TornadoDF(db)
    hail = HailDF(db)

    # 5) Provide the user with dataset choice
    ...
    if st.button("Close DB"):
        db.close()
        st.write("DataFrames cleared (nothing else to do).")
